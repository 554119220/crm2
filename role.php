...
/* 内部竞争 */
elseif ($_REQUEST['act'] == 'compete')
{
      $sql = 'UPDATE '.$ecs->table('role')." SET compete=IF(compete, 0, 1) WHERE role_id=$_POST[id]";
      if ($db->query($sql))
      {
            $sql = 'SELECT compete, role_id FROM '.$ecs->table('role')." WHERE role_id=$_POST[id]";
            $res = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'set_manager')
{
      if ($_POST['manager'])
      {
            $sql = 'UPDATE '.$GLOBALS['ecs']->table('role')." SET manager=$_POST[manager] WHERE role_id=$_POST[id]";
            $GLOBALS['db']->query($sql);
      }
      
      $sql = 'SELECT role_id, manager FROM '.$GLOBALS['ecs']->table('role')." WHERE role_id=$_POST[id]";
