...
else
{
    ...
    foreach ($role as $val)
    {
          $html_role[$val['role_id']] = mb_substr($val['role_name'], 0, 2, 'UTF-8');
    }

    /* 赋值到模板 */
    $smarty->assign('ur_here',          $_LANG['sell_stats']);
    $smarty->assign('role',             $html_role);
    $smarty->assign('goods_order_data', $goods_order_data['sales_order_data']);
    $smarty->assign('filter',           $goods_order_data['filter']);
    $smarty->assign('record_count',     $goods_order_data['record_count']);
    $smarty->assign('page_count',       $goods_order_data['page_count']);
...
function get_sales_order($is_pagination = true)
{
    ...
    $filter['team'] = empty($_REQUEST['team']) ? '' : trim($_REQUEST['team']);
    ...
    if ($filter['team'])
    {
          $sql = 'SELECT user_id FROM '.$GLOBALS['ecs']->table('admin_user').
                " WHERE role_id={$filter['team']} AND status=1";
          ...
          if (count($user_id))
          {
              ...
              $where .= ' AND oi.admin_id IN ('.implode(',', $uid).')';
          }
    }
