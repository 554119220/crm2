...
{
     $limit_time = time() -3*24*3600;
     $sql_select = 'SELECT * FROM '.$GLOBALS['ecs']->table('admin_message').
          " WHERE receiver_id={$_SESSION['admin_id']} AND sent_time>$limit_time";
     $res = $GLOBALS['db']->getAll($sql_select);
     foreach ($res as $val)
     {
          $msg .= '<p>'.$val['title'].'：'.$val['message'].'</p>';
     }
