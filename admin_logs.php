...
//将档案里的员工添加CRM管理账号
elseif($_REQUEST['act'] == 'syn_staff_admin'){
   $sql = 'SELECT user_id,user_name FROM '.$GLOBALS['ecs']->table('admin_user').' WHERE status=1'; 
   $admin_user = $GLOBALS['db']->getAll($sql);
   $sql = 'SELECT staff_id,staff_name FROM '.$GLOBALS['ecs']->table('oa_staff_records')." WHERE 1";
   $staff = $GLOBALS['db']->getAll($sql);

   foreach ($admin_user as $a) {
       foreach ($staff as $v) {
           if (0 == strcmp($a['user_name'],$v['staff_name'])) {
               $sql = 'UPDATE '.$GLOBALS['ecs']->table('oa_staff_records')." SET user_id={$a['user_id']} WHERE staff_id={$v['staff_id']}";
