...
function getRoleAdminListRes(res)
{
  var admin = res.admin;
  if (admin) {
    for (var t in res.target) {
      if (t != 'toJSONString') {
        var obj_select = document.getElementById(res.target[t]);
        obj_select.innerHTML = '';
        for(var i in admin)
        {
          if(admin[i].user_name != null)
          {
