...
if ($_REQUEST['act'] == 'num')
{
      ...
      $where .= ' AND u.user_id=a.user_id ';

      if ($_POST['age_group'])
      {
            $where .= " AND u.age_group='".$_POST['age_group']."' ";
      }
      $special_ranks = get_type_list($where); 
...
else
{
      ...
      case 'send_sms' :
            ...
            if ($_POST['district'])
            {
                  $where .= ' a.district='.$_POST['district'];
            }
            elseif ($_POST['city'])
            {
            ...
            $where != ' WHERE ' && $where = ', '.$GLOBALS['ecs']->table('user_address').' a '.$where.' AND  u.user_id=a.user_id AND ';

            if ($_POST['age_group'])
            {
                  $where .= " u.age_group='".$_POST['age_group']."' AND ";
            }
            ...
            if ($send_rank != 0)
            {
                  ...
                  if($rank_array['0'] == 1)
                  {
                        $sql = "SELECT mobile_phone $fields FROM " . $ecs->table('users') . " u $where u.mobile_phone <>'' AND u.admin_id=$_SESSION[admin_id]";
                        $row = $db->query($sql);
                  ...
                  {
                        $sql = "SELECT mobile_phone $fields FROM " . $ecs->table('users') . " u $where u.mobile_phone<>'' AND u.customer_type= '" . $rank_array['1'] . "' AND u.admin_id=$_SESSION[admin_id]";
                        $row = $db->query($sql);
                        while ($rank_rs = $db->fetch_array($row))
                        {
                              $value[] = $rank_rs['mobile_phone'];
