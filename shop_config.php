...
elseif ($_REQUEST['act'] == 'post')
{
    ...
    foreach ($_POST['value'] AS $key => $val)
    {
        if($arr[$key] != $val)
        {
             $sql = "UPDATE ".$ecs->table('shop_config')." SET value='".trim($val).
                  "' WHERE id='".$key."'";
            $db->query($sql);
        }
    }
            ...
            else
            {
                ...
                if (move_upload_file($file['tmp_name'], $file_name))
                {
                     $sql = "UPDATE ".$ecs->table('shop_config').
                          " SET value='$file_name' WHERE code='$code'";
                    $db->query($sql);
                }
                else
                {
    ...
    if (!empty($_POST['invoice_rate']))
    {
        ...
        $sql = "UPDATE ".$ecs->table('shop_config')." SET value = '".serialize($invoice).
