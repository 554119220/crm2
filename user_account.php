...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_paid == 1)
    {
        ...
        account_log_change($user_id, $amount, 0, 0, 0, $change_desc, $change_type);
    }
...
elseif ($_REQUEST['act'] == 'check')
{
    ...
    $res['main'] = $smarty->fetch('user_account_check.htm');
    die($json->encode($res));
}
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    $confirm_check_info = stripslashes($_REQUEST['JSON']);
     */

    $id = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;   //会员帐号明细id
    $is_paid = isset($_REQUEST['is_paid']) ? intval($_REQUEST['is_paid']) : 0;   //未确认 确认
    ...
    {
        $res['message'] = '未能找到用户或管理员备注为空，请重新确认';
        die($json->encode($res));
    }
    ...
    //如果状态为未确认
    if ($account['is_paid'] == 0)   //未付款
    {
        //如果是退款申请, 并且已完成,更新此条记录,扣除相应的余额
        if ($is_paid == '1' && $account['process_type'] == '1') //提现
        {
            ...
            //更新会员余额数量
            account_log_change($account['user_id'], $amount, 0, 0, 0, $_LANG['surplus_type_1'], ACT_DRAWING);
        }
        elseif ($is_paid == '1' && $account['process_type'] == '0') //充值
        {
            //如果是预付款，并且已完成, 更新此条记录，增加相应的余额
            update_user_account($id, $amount, $admin_note, $is_paid);

            //更新会员余额数量
            account_log_change($account['user_id'], $amount, 0, 0, 0, $_LANG['surplus_type_0'], ACT_SAVING);

        }
        elseif ($is_paid == '0') //已经付款
        {
            $sql = "UPDATE " .$ecs->table('user_account'). " SET ".
 ...
 * @return  void
 */
function account_log_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
    ...
    $result = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $account_log, 'INSERT');
