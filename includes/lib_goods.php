...
function goods_list($is_delete, $real_goods=1)
{
    ...
    if ($result === false)
    {
        ...
        $filter['filter'] = empty($_REQUEST['filter']) ? 0 : trim($_REQUEST['filter']);
        ...
        {
        case 'is_best':
            $where .= " AND g.is_best=1"; break;
        case 'is_hot':
            $where .= ' AND g.is_hot=1'; break;
        case 'is_new':
            $where .= ' AND g.is_new=1'; break;
        case 'is_promote':
            $where .= " AND g.is_promote=1 AND g.promote_price>0 AND g.promote_start_date<='$today' AND g.promote_end_date>='$today'"; break;
            case 'all_type';
            $where .= " AND (g.is_best=1 OR g.is_hot=1 OR g.is_new=1 OR (g.is_promote=1 AND g.promote_price>0 AND g.promote_start_date<='"
                .$today."' AND g.promote_end_date>='".$today."'))";
        }
        ...
        if ($filter['filter']) {
            ...
            if (!empty($goods_list)) {
                $where .= " AND g.goods_sn IN ($goods_list)";
            }elseif($filter['filter'] != 'sold_out'){
                $goods_list['record_count'] = 0;
                return $goods_list;
            }
        }
    ...
    //foreach ($row as $key=>&$val)
    //{
    //    $sql = 'SELECT IF(SUM(quantity), SUM(quantity), 0) goods_number, COUNT(quantity) stock_times FROM '.
    //        $GLOBALS['ecs']->table('stock_goods')." WHERE quantity>0 AND goods_sn='{$val['goods_sn']}'";
    //    $goods_res = $GLOBALS['db']->getRow($sql);
    //    $val['goods_number'] = $goods_res['goods_number'];
    //    $val['stock_times']  = $goods_res['stock_times'];
    //}
...
function brand_list ($is_show = 0)
{
    ...
    $sql_select .= 'ORDER BY sort_order ASC,brand_id ASC';
    $res = $GLOBALS['db']->getAll($sql_select);
    foreach ($res as &$val)
    {
        switch(strlen($val['brand_id']))
        {
        case 1: $val['brand_id'] = '00'.$val['brand_id']; break;
        case 2: $val['brand_id'] = '0'.$val['brand_id']; break;
        }
    }
 ...
 * 获取低库存商品 将过期商品
 */
function get_low_stock_goods($filter = 'low_qty')
{
    $low_stock_goods = array ();
    if ($filter == 'deadline') {
        $nowtime = time() +8*30*24*3600;
        $sql_select = 'SELECT DISTINCT s.goods_sn FROM '.$GLOBALS['ecs']->table('goods').' g,'.$GLOBALS['ecs']->table('stock_goods').
            " s WHERE expire_time<$nowtime AND g.is_delete=0 AND s.quantity>0";
        $low_stock_goods = $GLOBALS['db']->getCol($sql_select);
    } else {
        $sql_select = 'SELECT SUM(quantity) qty, g.warn_number, g.goods_sn FROM '.$GLOBALS['ecs']->table('stock_goods').
            ' s, '.$GLOBALS['ecs']->table('goods').' g WHERE g.goods_sn=s.goods_sn AND g.is_delete=0 GROUP BY s.goods_sn';
        $goods_list = $GLOBALS['db']->getAll($sql_select);

        foreach ($goods_list as $val){
            // 获取0库存的商品
            if ($filter == '0_qty' && $val['qty'] == 0) {
                $low_stock_goods[] = $val['goods_sn'];
            }

            // 获取低库存的商品
            if ($filter == 'low_qty' && ($val['qty'] <= $val['warn_number'] || $val['qty'] <= 20)) {
                $low_stock_goods[] = $val['goods_sn'];
            }
        }
    }

    $low_stock_goods = array_filter($low_stock_goods);
    return implode(',', $low_stock_goods);
}
 ...
 * @author Nixus
 **/
function get_goods_by_brand($brand_id)
{
    ...
    $sql_select = 'SELECT goods_name,goods_id FROM '.$GLOBALS['ecs']->table('goods')." WHERE brand_id=$brand_id";
    $goods_list = $GLOBALS['db']->getAll($sql_select);
