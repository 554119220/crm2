<?php
if (isset($_SESSION['admin_id']) && 1 != $_SESSION['admin_id']) {
    error_reporting(0);
}
...
require(ROOT_PATH . ADMIN_PATH . '/includes/cls_json.php');
...
if (!isset($_REQUEST['act'])) {
    ...
    $_REQUEST['act'] = '';
} elseif (($_REQUEST['act']=='forget_pwd'||$_REQUEST['act']=='get_pwd')&&strpos(PHP_SELF,'/get_password.php')===false) {
    $_REQUEST['act'] = '';
}
...
/* 验证管理员身份 */
if ((!isset($_SESSION['admin_id']) || intval($_SESSION['admin_id']) <= 0) && $_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' && $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order' && $_REQUEST['act'] != 'register' && $_REQUEST['act'] != 'submit_records')
{
    /* session 不存在，检查cookie */
    if (!empty($_COOKIE['ECSCP']['admin_id']) && !empty($_COOKIE['ECSCP']['admin_pass']))
    {
        // 找到了cookie, 验证cookie信息
        $sql = 'SELECT user_id,user_name,password,action_list,role_id,group_id,ext,last_login FROM ' .$ecs->table('admin_user') .
