...
class Images{
     ...
     function is_uploaded(){
          if(empty($_FILES[$this->inputName]['name'])){
               sys_msg($this->chk_err(4), 1);    //没有文件被上传
          }else{
               if(is_uploaded_file($_FILES[$this->inputName]['tmp_name'])){
                    return true;
               }else{
                    sys_msg($this->chk_err(-2), 1);       //文件上传不合法
               }
          }
     }
     ...
     function chk_type(){
          if(!in_array($_FILES[$this->inputName]['type'],$this->allowType)){
               sys_msg($this->chk_err(-3), 1);         //上传的文件类型不被允许
          }else{
               return true;
          }
     }
     ...
     function chk_size(){
          if($_FILES[$this->inputName]['size'] > $this->allowSize){
               sys_msg($this->chk_err(-4), 1);          //上传的文件过大
          }else{
               return true;
          }
     }
     ...
     function move_uploaded(){        //移动上传文件
          ...
          if($result){
               ...
               sys_msg($this->chk_err($_FILES[$this->inputName]['error']), 1);
          }
     ...
     function chk_err($errID){
          ...
          case -3:
               ...
               $this->errMsg = '暂时只支持jpg、jpeg、gif格式的图片';
               break;
          case -2:
               $this->errMsg = "文件上传不合法";
               break;
     ...
     * @return void
     ...
     function thumb($image,$is_save=true,$suofang=0,$type='',$maxWidth=500,$maxHeight=500,$interlace=true)
     {
          ...
          if($info !== false) {
               ...
               if (!in_array($type, $this->imgFormat)) sys_msg($this->chk_err(-3), 1);
               ...
               if ($suofang==1)
               {
                    ...
                    if ($_POST['w'] == 0) 
                         ...
                         $ratios_y = 210*$_POST['h']/$_POST['w'];

                    $thumbImg02 = imagecreatetruecolor(210,$ratios_y);
                    imagecopyresampled($thumbImg02,$thumbImg,0,0,$_POST['x'],$_POST['y'],210,$ratios_y,$_POST['w'],$_POST['h']);
